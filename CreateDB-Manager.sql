...
CREATE VIEW HiddenServicesToCrawle AS
  SELECT TOP 100 h.HiddenService
    ...
    WHERE DATEADD(hh, 12, p.LastCrawle)<SYSUTCDATETIME()
...
CREATE VIEW PagesToCrawle AS
  SELECT TOP 1000 Url
    ...
    WHERE LastCrawle<DATEADD(hh, -72, SYSUTCDATETIME())
    ORDER BY NEWID()
GO
...
CREATE VIEW LinkedMirrorsCandidate AS
  SELECT m.HiddenService,m.HiddenServiceTarget,p1.Title,SUBSTRING(p1.InnerText,0,128) InnerText, SUBSTRING(p2.InnerText ,0,128) AS InnerText2
      ,p1.CrawleError,p2.CrawleError AS CrawleError2
      ,r2.HiddenServiceMain AS HiddenServiceMainLoopWarning
      ,h1.IndexedPages, h2.IndexedPages as IndexedPages2
...
END
GO
GRANT EXECUTE ON MirrorsDetectTask TO SQL_Manager
GO
...
END
GO
GRANT EXECUTE ON UpdateHiddenServicesRankTask TO SQL_Manager
GO
...
BEGIN
  ...
  SELECT TOP 1000 Url, p.HiddenService, CrawleError, COALESCE(hd.Rank, 0.2) Rank
    INTO #RankingPages
...
END
GO
GRANT EXECUTE ON UpdatePageRankTask TO SQL_Manager
GO
...
BEGIN
  ...
  BEGIN
    SET @ret=1
    DELETE FROM Pages WHERE Url IN (SELECT TOP 1000 Url FROM Pages WHERE HiddenService=@url AND RankDate IS NOT NULL ORDER BY Rank ASC)
  END
  -- Banned management
  IF @ret=0
  BEGIN
...
END
GO
GRANT EXECUTE ON PagesPurgeTask TO SQL_Manager
GO
...
BEGIN
  ...
  MERGE HiddenServices AS target USING (SELECT HiddenService, COUNT(1) as IndexedPages FROM Pages WITH (NOLOCK) GROUP BY HiddenService) AS source ON (target.HiddenService = source.HiddenService)
    ...
    WHEN NOT MATCHED THEN INSERT (HiddenService,IndexedPages) VALUES (source.HiddenService,source.IndexedPages);

END
GO
GRANT EXECUTE ON ComputeIndexedPagesTask TO SQL_Manager
GO
